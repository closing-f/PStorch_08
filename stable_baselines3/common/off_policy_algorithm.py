...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def learn(
        ...
        arglist = parse_args()
        Save_Number = 0
        save_path = os.path.join(arglist.path_file, arglist.Model, arglist.reward_form,
                                 'seed_{}'.format(arglist.seed), '{}'.format(Save_Number))
        self.save(save_path)
        test_path = os.path.join(arglist.path_file, arglist.Model, arglist.reward_form,
                                 'seed_{}'.format(arglist.seed))
        eval_multiUAV(arglist=arglist, uav_env=self.env, model=self, path=test_path, ModelNumber=Save_Number)
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if self.num_timesteps % arglist.train_steps == 0:
                Save_Number += 1
                save_path = os.path.join(arglist.path_file, arglist.Model, arglist.reward_form,
                                         'seed_{}'.format(arglist.seed), '{}'.format(Save_Number))
                self.save(save_path)
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            # Warmup phase
            num = self.env.nr_agents
            # print("Warmup phase :{}".format(num))
            unscaled_action = []
            for idx in range(num):
                # print(idx)
                unscaled_action.append([self.action_space.sample()])
                # print(unscaled_action)
            unscaled_action = np.array(unscaled_action).flatten()
            unscaled_action = unscaled_action.reshape((num, 2))
            # print(unscaled_action)
        else:
    ...
    def collect_rollouts(
        ...
        while total_steps < n_steps or total_episodes < n_episodes:
            done = np.zeros(self.env.nr_agents)
            ...
            while not done[0]:
                ...
                # Select action randomly or according to policy
                # print("last_obs shape: {}".format(np.array(self._last_obs).shape))
                action, buffer_action = self._sample_action(learning_starts, action_noise)

                # Rescale and perform action
                # print("off policy actions shape: {}".format(action.shape))
